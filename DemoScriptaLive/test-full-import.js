...
var $author$project$Main$init = {input: '\\item{It provides a natural emergence of classical behavior from quantum mechanics}\n\n\\item{The measurement problem is partially resolved - definite outcomes emerge through environmental interaction}\n\n\\item{It explains why certain observables (like position) appear classical while others remain quantum}', output: ''};
	...
	function (msg, model) {
		if (msg.$ === 'InputChanged') {
			...
			return _Utils_update(
				model,
				{
					output: $author$project$Render$Export$LaTeXToScripta2$translate(model.input)
				});
		}
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Full LaTeX Import Test')
					])),
				A2(
				$elm$html$Html$h3,
				_List_Nil,
				_List_fromArray(
					[
						$elm$html$Html$text('Input LaTeX:')
					])),
				A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Convert to Scripta')
							]))
					])),
				A2(
				$elm$html$Html$h3,
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Output Scripta:')
					])),
				A2(
				$elm$html$Html$pre,
				_List_fromArray(
